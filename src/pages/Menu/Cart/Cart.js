  ...
  return (
    <>
      <div className={`mt-4 ${style.Body}`}>
        ...
        {props.cart.length > 0 ? (
          <>
            <div className={`mx-2 ${style.CartItemBody}`}>{display}</div>
            <div className={style.CartUtil}>
              <strong className={`mx-2 ${style.Price}`}>
                {" "}
                Total: ₹ {props.price}
              </strong>
              <Link to="/cart">
                <button className={style.CheckoutBtn}>GO TO CART</button>
              </Link>
            </div>
          </>
        ) : null}
      ...
      {props.cart.length > 0 ? (
        <Link to="/cart">
          <div className={style.BodyMobile}>GO TO CART</div>
        </Link>
      ) : null}
    </>
  );
